...
void thread_yield(void){
    ...
    if (setjmp(current_thread->env) == 0){ 
        ...
        // printf("Thread%d: task%d\n", current_thread->ID, current_thread->task_id);
        if (current_thread->task_id){ // if the current thread has task
            if (setjmp(current_thread->task_envs[current_thread->task_id]) == 0){
                // printf("Task%d start dipatch\n", current_thread->task_id);
                dispatch(); // dispatch the task
            }
        }
...
void pop(struct thread *t) {
    // if (t->stack_p >= (t->stack + 0x100*8-0x2*8)) {
